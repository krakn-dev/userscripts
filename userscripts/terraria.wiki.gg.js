...
(function () {
  ...
  function applyStickyNav() {
    ...
    // The sidebar's parent column is usually the element we want to make sticky.
    // MediaWiki Vector / Vector-2022 put the sidebar inside a flex/grid parent;
    // we want to make the sidebar itself scroll-sticky, not the parent column.
    ...
    Object.assign(sidebar.style, {
      ...
      overflowY: 'auto',
      overflowX: 'hidden',
      // Subtle custom scrollbar so it doesn't look broken
      scrollbarWidth: 'thin',
    });
  ...
  // Run immediately and re-run if the DOM is still loading sections
  ...
  // Also re-apply after any dynamic layout shifts (wiki.gg lazy-loads portlets)
  const observer = new MutationObserver(() => applyStickyNav());
  observer.observe(document.body, { childList: true, subtree: false });
  // Disconnect after 5 s – the sidebar won't change after initial load
